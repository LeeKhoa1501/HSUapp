...
import {
    ...
    Alert, ScrollView, RefreshControl, Dimensions, Image, ActivityIndicator, Linking
} from 'react-native';
...
// ANH NHỚ KIỂM TRA LẠI ĐƯỜNG DẪN ĐẾN CÁC COMPONENT NÀY CHO ĐÚNG
// <<< Import các component con đã tạo >>>
import HomeScreenHeader from '../screens/components/HomeScreenHeader'; // Giả sử đặt trong components/
import FavoriteFunctions from '../screens/components/FavoriteFunctions';
import TodayTimetableCard from '../screens/components/TodayTimetableCard';

// ANH NHỚ THAY BẰNG IP VÀ PORT ĐÚNG CỦA BACKEND SERVER
const BASE_URL = 'http://10.101.38.213:5000';

const screenWidth = Dimensions.get('window').width;
const functionGroupItemSize = (screenWidth - 2 * 20 - (4 - 1) * 10) / 4;
...
const HomeScreen = () => {
  ...
  const loadFavoriteFunctionIds = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) setFavoriteFunctionIds([16, 5, 4, 6]);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Bỏ 'Sự kiện & Khác' nếu card mới thay thế hoàn toàn
    // const eventOtherFunctions = homeData.functions.filter(f => [13].includes(f.id));
    setGroupedFunctions({
      'Thông tin học vụ': mainAcademicFunctions,
      'Khảo sát & Dịch vụ': surveyServiceFunctions,
      // 'Sự kiện & Khác': eventOtherFunctions,
    });
    loadAuthData();
  ...
  const onRefresh = useCallback(async () => {
    ...
    if (userToken && isMountedRef.current) {
        ...
        await fetchAttendedEventsSummary();
    }
  ...
  const renderFunctionGroupItem = ({ item }) => (
    <TouchableOpacity style={styles.functionGroupItem} onPress={() => handleFunctionPress(item)}>
      <View style={styles.functionGroupIconContainer}>
        <FontAwesome5 name={item.icon} size={functionGroupItemSize * 0.4} color={item.iconColor || "#003366"} />
      </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        {Object.keys(groupedFunctions).map((sectionTitle) => (
          // Kiểm tra xem nhóm có chức năng không và không phải là nhóm "Sự kiện & Khác" cũ (nếu có)
        ...
        <View style={styles.generalCardContainer}>
          <View style={styles.attendedEventsHeader}>
            ...
            {/* Optional: Nút xem tất cả sự kiện đã tham gia */}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  {item.status === 'Đã kết thúc' && ( // Demo badge "Đã diễn ra"
...
const styles = StyleSheet.create({
  ...
  favoriteHeaderStyle: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, marginTop: 20 },
  editFavoritesButton: { padding: 8 },
  sectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#002366' },
  generalCardContainer: {
    backgroundColor: '#ffffff', borderRadius: 12, paddingHorizontal: 15, paddingTop:15, paddingBottom:10,
    marginHorizontal: 20, marginTop: 20, shadowColor: '#000',
  ...
  attendedEventItem: {
    width: screenWidth * 0.6, height: screenWidth * 0.38, marginRight: 12,
    borderRadius: 8, backgroundColor: '#e9ecef', elevation: 2,
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.15, shadowRadius: 2.5,
  },
  ...
  attendedEventName: { fontSize: 13, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 3 },
  attendedEventDate: { fontSize: 11, color: '#E0E0E0' },
  attendedBadge: {
  ...
  functionSectionCard: {
    ...
    marginHorizontal: 20, shadowColor: '#000',
  ...
  functionSectionHeader: {
    ...
    alignItems: 'center', paddingHorizontal: 15, marginBottom: 5,
  },
  horizontalListContainer: { paddingLeft: 15, paddingRight: 5 },
  functionGroupItem: {
    width: functionGroupItemSize, alignItems: 'center',
    justifyContent: 'flex-start', marginRight: 10, marginBottom: 10,
  },
  functionGroupIconContainer: {
    width: functionGroupItemSize * 0.7, height: functionGroupItemSize * 0.7,
    borderRadius: 12, backgroundColor: '#F0F5FF',
    justifyContent: 'center', alignItems: 'center', marginBottom: 8,
  },
  functionGroupText: {
    textAlign: 'center', fontSize: 11, color: '#34495e',
    lineHeight: 14, fontWeight: '500', height: 28,
  },
});
